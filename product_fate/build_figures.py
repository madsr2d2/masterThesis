...
def build_curves_page():
    ...
    body = (f"<p class='lede'>All {len(panels)} live cuvettes of the "
            ...
            "(<code>figure_kit.progress_panel</code>). The long-dashed line from the curve's start is its tangent at t = 0, whose slope is the fitted initial rate <code>v(0) = v_ss − ΣB/τ</code>, labelled on the derivative strip; on a one-phase fit it crosses the asymptote exactly at τ. "
            "<code>v_peak</code> is "
