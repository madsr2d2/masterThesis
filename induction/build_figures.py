...
def build_curves_page():
    ...
    body = (f"<p class='lede'>Every live 4OMe cuvette in the archive, both "
            ...
            "at t = 0. The long-dashed line from the curve's start is its tangent at t = 0, whose slope is the fitted initial rate <code>v(0) = v_ss − ΣB/τ</code>, labelled on the derivative strip; on a one-phase fit it crosses the asymptote exactly at τ. On a lag it is the rate the induction starts "
            "from. Nothing is excluded.</p>"
...
disagree.</p>
"""
    return styled("What happens before the 4OMe curves start", body,
                  "The catalyst wakes up on its own clock")
