...
def main():
    ...
    # nothing.
    doc.claim("the source split",
          f"**{int((frame.source == 'rre').sum())} of {len(frame)} come from "
              f"the instrument's own `.rre`**")
    doc.check("no curve in the series falls back to the export",
    ...
    doc.check("nor anywhere in the archive",
              not fallbacks,
              f"{len(fallbacks)} of {len(everything)} curves on .txt")
    doc.claim("the recovery",
    ...
    doc.check("seven distinct stocks", len(stocks) == 7, f"{len(stocks)}")
    doc.claim("chains self-consistent",
              f"**{int(used.chain_ok.sum())} of the {len(used)} chains are "
              f"self-consistent**")
    doc.claim("compiled equals the sheet",
          f"**{int(used.agrees.sum())} of {len(used)} compiled values equal "
              f"the sheet's own `kuv`**")
    for _, row in stocks.iterrows():
        doc.claim(f"stock at {row.grams:g} g",
    ...
    doc.check("exp 16 is the only out-of-sequence stock",
    ...
    for estimator in ("vmax", "v0_quad"):
        ...
        doc.check(f"{estimator}: the recorded values win",
        ...
        if estimator == "vmax":
            for name, label in (("recorded", "**as recorded**"),
                ...
                doc.claim(f"hypothesis row, {name}",
    ...
    doc.claim("as recorded", f"as recorded (0.241): **{plain.loc[16, 'mean_residual']:+.3f}**")
    doc.claim("forced",
              f"forced to 0.273: **{forced.loc[16, 'mean_residual']:+.3f}**")
    doc.claim("the neighbours' scatter",
    ...
    doc.check("forcing 0.273 moves exp 16 further out",
    ...
    doc.claim("the run lengths in tau",
    ...
    doc.claim("tau falls as it warms", "**" + ladder + "**")
    ...
    drawn = normalise(io.open(os.path.join(HERE, "build_figures.py"),
                              encoding="utf-8").read())
    doc.check("figure E's caption carries the same ladder",
                  normalise(ladder) in drawn, ladder)
    doc.check("every run below 32 C is a lag curve",
    ...
    for pattern in patterns:
        ...
        doc.claim(f"pattern row, {pattern}",
                  f"| {pattern} | " + " | ".join(
                      f"{mark}{v}{mark}" if (mark and v) else str(v)
                      for v in row) + " |")
    two = int((frame.break_count == 2).sum())
    doc.claim("how many take a second breakpoint",
    ...
    doc.claim("exp 16's early break",
    ...
    doc.check("the model-free pattern and the nested fit partition the block alike",
              agree,
    ...
    doc.claim("tau resolved, cold end",
              f"tau resolved on {int(warm.tau_resolved.sum())} of {len(warm)} curves")
    doc.claim("tau unresolved, hot end",
    ...
    ladder = frame.pivot_table(index="temperature", columns="sample",
                               values="buf")
    doc.check("[buf] falls 80-50 along the ladder, identically in all six runs",
    ...
    for name, row in table.iterrows():
        ...
        doc.claim(f"parameter row, {name}",
    ...
    doc.claim("the rung agreement cost",
    ...
    doc.claim("the composition the entropy is quoted at",
    ...
    doc.claim("how many earned the second phase",
    ...
    for temperature in sorted(frame_sel.temperature.unique()):
        ...
        mark = "**" if two else ""
        doc.claim(f"selection row, {temperature:.0f} C",
    ...
    doc.check("nothing is selected where one phase already fits",
    ...
    for other in ("vmax", "v_ss"):
        doc.check(f"v_peak and {other} agree within their errors",
    ...
    doc.claim("the order in catalyst",
    ...
    doc.check("catalyst inactivation is excluded by the direction of the effect",
    ...
    doc.claim("how far above v_peak the production rate sits",
              f"**{effect['lift'] - 1:.1%} above**")
    doc.claim("its range over the six temperatures",
              f"between {effect['lift_low'] - 1:+.1%} and "
              f"{effect['lift_high'] - 1:+.1%}")
    doc.claim("the shift in the activation energy",
              f"**{effect['activation_shift']:+.2f} +/- "
              f"{effect['activation_shift_stderr']:.2f} kJ/mol**")
    doc.claim("the shift in the entropy",
              f"**{effect['entropy_from_lift']:+.2f} J/mol/K**")
    doc.check("the corrected route is the noisier one",
    ...
    fixed = slowdown.deceleration_drivers(named["4OMe catalysed, phosphate"],
                                          fixed=True)
    doc.claim("the block's own product coefficient",
              f"{series['product']:+.3f} +/- {series['product_stderr']:.3f}")
    doc.claim("against its run-length coefficient",
              f"{series['span']:+.3f} +/- {series['span_stderr']:.3f}")
    doc.claim("the enzyme-free block's run-length coefficient",
              f"{free['span']:+.3f} +/- {free['span_stderr']:.3f}")
    doc.claim("and its product coefficient",
              f"{free['product']:+.3f} +/- {free['product_stderr']:.3f}")
    doc.claim("the every-condition-fixed comparison",
    ...
    doc.claim("the plateau's substrate order",
    ...
    doc.claim("the selectivity it implies",
              f"k_A/k_S ~ {np.median(picked):.0f}")
    counts = clean.prefers.value_counts()
    doc.claim("how many curves prefer the sink form",
    ...
    letters = doc.figures(os.path.join(HERE, "index.html"))
    doc.claim("the document's own count of them",
    ...
    doc.claim("how many curves the burst form cannot hold",
              f"{two_phase} of these {len(series)} curves do exactly that")
    doc.check("the model-free screen agrees with the F test on that count",
    ...
    doc.check("the fit is narrower than a mark on every panel",
                  worst is not None and worst > 0,
                  f"{worst:.2f} px of margin at worst" if worst is not None
                  else "no panel carried both a mark and a fit line")
    ...
    doc.check("no white halo is drawn over the readings", not haloed,
              f"{len(haloed)} panels")
