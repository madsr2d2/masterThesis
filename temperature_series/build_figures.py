...
def build_curves_page():
    ...
    body = ("<p class='lede'>All 24 curves of the temperature series, in "
            ...
            "its distance from the curve at t = 0, τ as a rule. The long-dashed line from the curve's start is its tangent at t = 0, whose slope is the fitted initial rate <code>v(0) = v_ss − ΣB/τ</code>, labelled on the derivative strip; on a one-phase fit it crosses the asymptote exactly at τ. "
            "On this series it is mostly the induction rate rather than the "
            "reaction's: most of these curves accelerate, so t = 0 is where "
            "they are slowest.</p>"
